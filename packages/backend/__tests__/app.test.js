...
describe('API Endpoints', () => {
  ...
  describe('DELETE /api/items/:id', () => {
    ...
    it('should verify item is actually deleted from database', async () => {
      ...
      await request(app).delete(`/api/items/${createdItemId}`).set('Accept', 'application/json');
    ...
    it('should handle multiple deletions correctly', async () => {
      ...
      const getResponse = await request(app).get('/api/items');
      const remainingItems = getResponse.body.filter(
        item => item.id === createdItemId || item.id === secondItemId
      );
